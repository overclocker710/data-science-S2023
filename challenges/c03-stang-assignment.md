...
df_stang_long <-
  ...
  pivot_longer(
    ...
    names_sep = "_") %>%
  #convert the angle column to numeric
  mutate(angle = as.integer(angle)) %>%
  #filter out any invalid values (e.g. NA, negative, etc.)
  filter(!is.na(angle), angle >= 0, angle <= 90, 
         !is.na(E), E > 0, 
         !is.na(mu), mu >= 0, mu <= 0.5)
...
- Is there “one true value” for the material properties of Aluminum?
  - Not really, there is an average and you can give a range a la
    boxplot, but the mean doesn’t really do it justice, there is a
    substantial amount of variation.
- How many aluminum alloys are in this dataset? How do you know?
  - There is only 1, I looked at all the rows of the dataset and its
    just the one alloy
- What angles were tested?
  - 0, 45, and 90.
- What thicknesses were tested?
  - 0.022 to 0.081 inches.
- Does thickness or angle affect elasticity more.
...
df_stang_long %>%
  ...
  mutate(
    thiccut = cut_interval(thick, 6)
  ) %>% 
  ggplot(aes(x = thiccut, y = E)) +
  geom_boxplot()
```
...
df_stang_long %>%
  ## TODO: Use mutate to add the column `carat_cut`
  mutate(
    angle_cut = cut_interval(angle, 6)
  ) %>% 
  ggplot(aes(x = angle_cut, y = E)) +
  geom_boxplot()
```

![](c03-stang-assignment_files/figure-gfm/q3-vis-2-1.png)<!-- -->

**Observations**:

- The interquartile range when graphing angle has so much overlap, in
  addition to the fact that the medians display no trend, that I would
  have to say angle does not affect elasticity. Thickness, however, does
  have a meaningful impact, even including outliers, there is no overlap
  between the thickest group and any of the other groups, it is too far
  below them. Additionally among the 3 thinner groups they also display
  the same trend to a lesser extent.
...
- Does this graph support or contradict the claim above?
  - This evidence in addition to my evidence suggests a contradiction of
    the above statement, however I would venture to say that this
    statement can still be trusted. The variation in elasticity caused
    by a factor of 4 in the thickness is only 5%. From a practical
    perspective you can say thickness doesn’t affect elasticity, but it
    does have a small but measurable impact.
