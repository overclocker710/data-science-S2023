...
-   There are a total of 35 simulations. Most columns don't have massive variation and seem to vary within the same order of magnitude. For the most part we should therefore be OK to use linear scales for visualizations as opposed to log scales.
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   the cheat model is the most accurate and the nonphysical model is the least accurate.
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is a quantitative variable that represents temperature of the fluid averaged over a plane. Being an average it encapsulates a lot of information, and I believe this average includes the region where T_norm is measured, so of course it is a good predictor.
...
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx can be considered an identifier variable. It has no physical meaning and is simply used to identify and organized the multiple trials that occurred.
...
-   Based on looking at what each variable represented, I tried a few different variables and found out that x, W, L, and U_0 seem to be the best combination. I though I_0 or eps_p would work really well, but surprisingly they made it slightly worse and independently were unable to predict to any meaningful degree, so I dumped them.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   My model from q4 seems to be more accurate because it follows the line more closely
-   Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
    -   My model from q4 is more confident in its predictions, it has narrower prediction interval bars.
-   How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
    -   fit_simple only needs one predictor, while my fit_q4 needs four of them.
...
-   How much do you trust your model? Why?
    -   I trust it quite a bit, I gave it all relevant data that I could, and tested many versions and this came out on top. Also it does have prediction intervals so I trust it as much as the prediction intervals specify.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   Prediction intervals, because we are making new predictions we should be using prediction intervals to see the uncertainty in the range of possible predictions
-   What fraction of validation cases lie within the interval you predict? How does this compare with `pr_level`?
    -   93.3%, which is a very high percentage, far higher than the 80% we requested which means this prediction interval is conservative.
-   What interval for `T_norm` would you recommend the design team to plan around?
    -   1.46 to 2.30, this is what our model predicts, with at least 80% confidence.
-   Are there any other recommendations you would provide?
    -   T_norm will likely not even vary that much based on the validation data test, showing that 80% probability is conservative and it is likely above 90% real probability.
