...
```
    ...
    ##       x   idx     L      W   U_0     N_p    k_f   T_f rho_f    mu_f  lam_f  C_fp
    ##   <dbl> <dbl> <dbl>  <dbl> <dbl>   <dbl>  <dbl> <dbl> <dbl>   <dbl>  <dbl> <dbl>
    ## 1     1    30 0.153 0.0403  1.69  2.24e6 0.100   360. 1.49  1.68e-5 0.0327 1053.
    ## 2     1    31 0.172 0.0329  1.95  1.72e6 0.109   253. 1.20  1.94e-5 0.0334 1104.
    ## 3     1    32 0.137 0.0446  2.34  2.10e6 0.0964  281. 0.995 2.08e-5 0.0297  943.
    ## 4     1    33 0.188 0.0383  2.14  1.96e6 0.119   262. 1.30  1.89e-5 0.0285  989.
    ## 5     1    34 0.150 0.0364  2.56  1.84e6 0.0845  272. 1.27  1.60e-5 0.0291  929.
    ## 6     1    35 0.168 0.0493  1.78  2.39e6 0.113   324. 1.15  1.64e-5 0.0248 1213.
    ## # … with 10 more variables: rho_p <dbl>, d_p <dbl>, C_pv <dbl>, h <dbl>,
    ## #   I_0 <dbl>, eps_p <dbl>, avg_q <dbl>, avg_T <dbl>, rms_T <dbl>, T_norm <dbl>
...
- There are a total of 35 simulations. Most columns don’t have massive
  variation and seem to vary within the same order of magnitude. For the
  most part we should therefore be OK to use linear scales for
  visualizations as opposed to log scales.
...
df_validate <-
  df_validate %>% 
  mutate(
    pred_baseline = predict(fit_baseline, newdata = .),
    pred_cheat = predict(fit_cheat, newdata = .),
    pred_nonphysical = predict(fit_nonphysical, newdata = .)
  )

mse_baseline <-
  mean((df_validate$T_norm - df_validate$pred_baseline)^2)
mse_cheat <-
  mean((df_validate$T_norm - df_validate$pred_cheat)^2)
mse_nonphysical <-
  mean((df_validate$T_norm - df_validate$pred_nonphysical)^2)
...
- Which model is *most accurate*? Which is *least accurate*?
  - the cheat model is the most accurate and the nonphysical model is
    the least accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_T is a quantitative variable that represents temperature of the
    fluid averaged over a plane. Being an average it encapsulates a lot
    of information, and I believe this average includes the region where
    T_norm is measured, so of course it is a good predictor.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No, we wouldn’t. They are intertwined.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx can be considered an identifier variable. It has no physical
    meaning and is simply used to identify and organized the multiple
    trials that occurred.
...
- Based on looking at what each variable represented, I tried a few
  different variables and found out that x, W, L, and U_0 seem to be the
  best combination. I though I_0 or eps_p would work really well, but
  surprisingly they made it slightly worse and independently were unable
  to predict to any meaningful degree, so I dumped them.
...
``` r
pred_q6 <- predict(fit_q6, newdata = df_validate)

pred_uncertainty <-
  add_uncertainties(df_design, fit_q6, interval = "prediction", prefix = "pi", level = pr_level)

df_validate %>% 
  add_uncertainties(fit_q6, interval = "prediction", prefix = "pi", level = pr_level) %>% 
  mutate(
    within_interval = T_norm >= pi_lwr & T_norm <= pi_upr
    ) %>% 
  summarise(percent_within_interval = mean(within_interval) * 100) %>% 
  pull(percent_within_interval)
```

    ## [1] 93.33333

``` r
lower_bound <- pred_q6 - pred_uncertainty
upper_bound <- pred_q6 + pred_uncertainty
...
``` r
#cat("Dependable range of values for T_norm at the point df_design: [", lower_bound, ",", upper_bound, "]\n")
```

**Recommendation**:

- How much do you trust your model? Why?
  - I trust it quite a bit, I gave it all relevant data that I could,
    and tested many versions and this came out on top. Also it does have
    prediction intervals so I trust it as much as the prediction
    intervals specify.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - Prediction intervals, because we are making new predictions we
    should be using prediction intervals to see the uncertainty in the
...
- What fraction of validation cases lie within the interval you predict?
  ...
  - 93.3%, which is a very high percentage, far higher than the 80% we
    requested which means this prediction interval is conservative.
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - 1.46 to 2.30, this is what our model predicts, with at least 80%
    confidence.
- Are there any other recommendations you would provide?
  - T_norm will likely not even vary that much based on the validation
    data test, showing that 80% probability is conservative and it is
    likely above 90% real probability.
